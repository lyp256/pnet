...
<body>
<div>this bar server</div>
<div><a href="./switch_foo">switch to foo server</a></div>
...
func main() {
	...
	mux.HandleFunc("/switch_foo", func(writer http.ResponseWriter, request *http.Request) {
		execPath, err := exec.LookPath("./foo")
		if err != nil {
			fmt.Fprintf(writer, "get foo failed:%s", err)
			return
		}
		cmd := exec.Command(execPath)
		err = cmd.Start()
		if err != nil {
			fmt.Fprintf(writer, "foo start failed:%s", err)
			return
		}
		go ser.Shutdown(context.Background())
		fmt.Fprintf(writer, "foo start success")
		return
	})
